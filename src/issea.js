let table = [
  [
    -90,
    -180
  ],
  [
    -90,
    -177
  ],
  [
    -90,
    -174
  ],
  [
    -90,
    -171
  ],
  [
    -90,
    -168
  ],
  [
    -90,
    -165
  ],
  [
    -90,
    -162
  ],
  [
    -90,
    -159
  ],
  [
    -90,
    -156
  ],
  [
    -90,
    -153
  ],
  [
    -90,
    -150
  ],
  [
    -90,
    -147
  ],
  [
    -90,
    -144
  ],
  [
    -90,
    -141
  ],
  [
    -90,
    -138
  ],
  [
    -90,
    -135
  ],
  [
    -90,
    -132
  ],
  [
    -90,
    -129
  ],
  [
    -90,
    -126
  ],
  [
    -90,
    -123
  ],
  [
    -90,
    -120
  ],
  [
    -90,
    -117
  ],
  [
    -90,
    -114
  ],
  [
    -90,
    -111
  ],
  [
    -90,
    -108
  ],
  [
    -90,
    -105
  ],
  [
    -90,
    -102
  ],
  [
    -90,
    -99
  ],
  [
    -90,
    -96
  ],
  [
    -90,
    -93
  ],
  [
    -90,
    -90
  ],
  [
    -90,
    -87
  ],
  [
    -90,
    -84
  ],
  [
    -90,
    -81
  ],
  [
    -90,
    -78
  ],
  [
    -90,
    -75
  ],
  [
    -90,
    -72
  ],
  [
    -90,
    -69
  ],
  [
    -90,
    -66
  ],
  [
    -90,
    -63
  ],
  [
    -90,
    -60
  ],
  [
    -90,
    -57
  ],
  [
    -90,
    -54
  ],
  [
    -90,
    -51
  ],
  [
    -90,
    -48
  ],
  [
    -90,
    -45
  ],
  [
    -90,
    -42
  ],
  [
    -90,
    -39
  ],
  [
    -90,
    -36
  ],
  [
    -90,
    -33
  ],
  [
    -90,
    -30
  ],
  [
    -90,
    -27
  ],
  [
    -90,
    -24
  ],
  [
    -90,
    -21
  ],
  [
    -90,
    -18
  ],
  [
    -90,
    -15
  ],
  [
    -90,
    -12
  ],
  [
    -90,
    -9
  ],
  [
    -90,
    -6
  ],
  [
    -90,
    -3
  ],
  [
    -90,
    0
  ],
  [
    -90,
    3
  ],
  [
    -90,
    6
  ],
  [
    -90,
    9
  ],
  [
    -90,
    12
  ],
  [
    -90,
    15
  ],
  [
    -90,
    18
  ],
  [
    -90,
    21
  ],
  [
    -90,
    24
  ],
  [
    -90,
    27
  ],
  [
    -90,
    30
  ],
  [
    -90,
    33
  ],
  [
    -90,
    36
  ],
  [
    -90,
    39
  ],
  [
    -90,
    42
  ],
  [
    -90,
    45
  ],
  [
    -90,
    48
  ],
  [
    -90,
    51
  ],
  [
    -90,
    54
  ],
  [
    -90,
    57
  ],
  [
    -90,
    60
  ],
  [
    -90,
    63
  ],
  [
    -90,
    66
  ],
  [
    -90,
    69
  ],
  [
    -90,
    72
  ],
  [
    -90,
    75
  ],
  [
    -90,
    78
  ],
  [
    -90,
    81
  ],
  [
    -90,
    84
  ],
  [
    -90,
    87
  ],
  [
    -90,
    90
  ],
  [
    -90,
    93
  ],
  [
    -90,
    96
  ],
  [
    -90,
    99
  ],
  [
    -90,
    102
  ],
  [
    -90,
    105
  ],
  [
    -90,
    108
  ],
  [
    -90,
    111
  ],
  [
    -90,
    114
  ],
  [
    -90,
    117
  ],
  [
    -90,
    120
  ],
  [
    -90,
    123
  ],
  [
    -90,
    126
  ],
  [
    -90,
    129
  ],
  [
    -90,
    132
  ],
  [
    -90,
    135
  ],
  [
    -90,
    138
  ],
  [
    -90,
    141
  ],
  [
    -90,
    144
  ],
  [
    -90,
    147
  ],
  [
    -90,
    150
  ],
  [
    -90,
    153
  ],
  [
    -90,
    156
  ],
  [
    -90,
    159
  ],
  [
    -90,
    162
  ],
  [
    -90,
    165
  ],
  [
    -90,
    168
  ],
  [
    -90,
    171
  ],
  [
    -90,
    174
  ],
  [
    -90,
    177
  ],
  [
    -90,
    180
  ],
  [
    -87,
    -180
  ],
  [
    -87,
    -177
  ],
  [
    -87,
    -174
  ],
  [
    -87,
    -171
  ],
  [
    -87,
    -168
  ],
  [
    -87,
    -165
  ],
  [
    -87,
    -162
  ],
  [
    -87,
    -159
  ],
  [
    -87,
    -156
  ],
  [
    -87,
    -153
  ],
  [
    -87,
    -150
  ],
  [
    -87,
    -147
  ],
  [
    -87,
    -144
  ],
  [
    -87,
    -141
  ],
  [
    -87,
    -138
  ],
  [
    -87,
    -135
  ],
  [
    -87,
    -132
  ],
  [
    -87,
    -129
  ],
  [
    -87,
    -126
  ],
  [
    -87,
    -123
  ],
  [
    -87,
    -120
  ],
  [
    -87,
    -117
  ],
  [
    -87,
    -114
  ],
  [
    -87,
    -111
  ],
  [
    -87,
    -108
  ],
  [
    -87,
    -105
  ],
  [
    -87,
    -102
  ],
  [
    -87,
    -99
  ],
  [
    -87,
    -96
  ],
  [
    -87,
    -93
  ],
  [
    -87,
    -90
  ],
  [
    -87,
    -87
  ],
  [
    -87,
    -84
  ],
  [
    -87,
    -81
  ],
  [
    -87,
    -78
  ],
  [
    -87,
    -75
  ],
  [
    -87,
    -72
  ],
  [
    -87,
    -69
  ],
  [
    -87,
    -66
  ],
  [
    -87,
    -63
  ],
  [
    -87,
    -60
  ],
  [
    -87,
    -57
  ],
  [
    -87,
    -54
  ],
  [
    -87,
    -51
  ],
  [
    -87,
    -48
  ],
  [
    -87,
    -45
  ],
  [
    -87,
    -42
  ],
  [
    -87,
    -39
  ],
  [
    -87,
    -36
  ],
  [
    -87,
    -33
  ],
  [
    -87,
    -30
  ],
  [
    -87,
    -27
  ],
  [
    -87,
    -24
  ],
  [
    -87,
    -21
  ],
  [
    -87,
    -18
  ],
  [
    -87,
    -15
  ],
  [
    -87,
    -12
  ],
  [
    -87,
    -9
  ],
  [
    -87,
    -6
  ],
  [
    -87,
    -3
  ],
  [
    -87,
    0
  ],
  [
    -87,
    3
  ],
  [
    -87,
    6
  ],
  [
    -87,
    9
  ],
  [
    -87,
    12
  ],
  [
    -87,
    15
  ],
  [
    -87,
    18
  ],
  [
    -87,
    21
  ],
  [
    -87,
    24
  ],
  [
    -87,
    27
  ],
  [
    -87,
    30
  ],
  [
    -87,
    33
  ],
  [
    -87,
    36
  ],
  [
    -87,
    39
  ],
  [
    -87,
    42
  ],
  [
    -87,
    45
  ],
  [
    -87,
    48
  ],
  [
    -87,
    51
  ],
  [
    -87,
    54
  ],
  [
    -87,
    57
  ],
  [
    -87,
    60
  ],
  [
    -87,
    63
  ],
  [
    -87,
    66
  ],
  [
    -87,
    69
  ],
  [
    -87,
    72
  ],
  [
    -87,
    75
  ],
  [
    -87,
    78
  ],
  [
    -87,
    81
  ],
  [
    -87,
    84
  ],
  [
    -87,
    87
  ],
  [
    -87,
    90
  ],
  [
    -87,
    93
  ],
  [
    -87,
    96
  ],
  [
    -87,
    99
  ],
  [
    -87,
    102
  ],
  [
    -87,
    105
  ],
  [
    -87,
    108
  ],
  [
    -87,
    111
  ],
  [
    -87,
    114
  ],
  [
    -87,
    117
  ],
  [
    -87,
    120
  ],
  [
    -87,
    123
  ],
  [
    -87,
    126
  ],
  [
    -87,
    129
  ],
  [
    -87,
    132
  ],
  [
    -87,
    135
  ],
  [
    -87,
    138
  ],
  [
    -87,
    141
  ],
  [
    -87,
    144
  ],
  [
    -87,
    147
  ],
  [
    -87,
    150
  ],
  [
    -87,
    153
  ],
  [
    -87,
    156
  ],
  [
    -87,
    159
  ],
  [
    -87,
    162
  ],
  [
    -87,
    165
  ],
  [
    -87,
    168
  ],
  [
    -87,
    171
  ],
  [
    -87,
    174
  ],
  [
    -87,
    177
  ],
  [
    -87,
    180
  ],
  [
    -84,
    -180
  ],
  [
    -84,
    -177
  ],
  [
    -84,
    -174
  ],
  [
    -84,
    -171
  ],
  [
    -84,
    -168
  ],
  [
    -84,
    -165
  ],
  [
    -84,
    -162
  ],
  [
    -84,
    -159
  ],
  [
    -84,
    -156
  ],
  [
    -84,
    -153
  ],
  [
    -84,
    -150
  ],
  [
    -84,
    -147
  ],
  [
    -84,
    -144
  ],
  [
    -84,
    -141
  ],
  [
    -84,
    -138
  ],
  [
    -84,
    -135
  ],
  [
    -84,
    -132
  ],
  [
    -84,
    -129
  ],
  [
    -84,
    -126
  ],
  [
    -84,
    -123
  ],
  [
    -84,
    -120
  ],
  [
    -84,
    -117
  ],
  [
    -84,
    -114
  ],
  [
    -84,
    -111
  ],
  [
    -84,
    -108
  ],
  [
    -84,
    -105
  ],
  [
    -84,
    -102
  ],
  [
    -84,
    -99
  ],
  [
    -84,
    -96
  ],
  [
    -84,
    -93
  ],
  [
    -84,
    -90
  ],
  [
    -84,
    -87
  ],
  [
    -84,
    -84
  ],
  [
    -84,
    -81
  ],
  [
    -84,
    -78
  ],
  [
    -84,
    -75
  ],
  [
    -84,
    -72
  ],
  [
    -84,
    -69
  ],
  [
    -84,
    -66
  ],
  [
    -84,
    -63
  ],
  [
    -84,
    -60
  ],
  [
    -84,
    -57
  ],
  [
    -84,
    -54
  ],
  [
    -84,
    -51
  ],
  [
    -84,
    -48
  ],
  [
    -84,
    -45
  ],
  [
    -84,
    -42
  ],
  [
    -84,
    -39
  ],
  [
    -84,
    -36
  ],
  [
    -84,
    -33
  ],
  [
    -84,
    -30
  ],
  [
    -84,
    -27
  ],
  [
    -84,
    -24
  ],
  [
    -84,
    -21
  ],
  [
    -84,
    -18
  ],
  [
    -84,
    -15
  ],
  [
    -84,
    -12
  ],
  [
    -84,
    -9
  ],
  [
    -84,
    -6
  ],
  [
    -84,
    -3
  ],
  [
    -84,
    0
  ],
  [
    -84,
    3
  ],
  [
    -84,
    6
  ],
  [
    -84,
    9
  ],
  [
    -84,
    12
  ],
  [
    -84,
    15
  ],
  [
    -84,
    18
  ],
  [
    -84,
    21
  ],
  [
    -84,
    24
  ],
  [
    -84,
    27
  ],
  [
    -84,
    30
  ],
  [
    -84,
    33
  ],
  [
    -84,
    36
  ],
  [
    -84,
    39
  ],
  [
    -84,
    42
  ],
  [
    -84,
    45
  ],
  [
    -84,
    48
  ],
  [
    -84,
    51
  ],
  [
    -84,
    54
  ],
  [
    -84,
    57
  ],
  [
    -84,
    60
  ],
  [
    -84,
    63
  ],
  [
    -84,
    66
  ],
  [
    -84,
    69
  ],
  [
    -84,
    72
  ],
  [
    -84,
    75
  ],
  [
    -84,
    78
  ],
  [
    -84,
    81
  ],
  [
    -84,
    84
  ],
  [
    -84,
    87
  ],
  [
    -84,
    90
  ],
  [
    -84,
    93
  ],
  [
    -84,
    96
  ],
  [
    -84,
    99
  ],
  [
    -84,
    102
  ],
  [
    -84,
    105
  ],
  [
    -84,
    108
  ],
  [
    -84,
    111
  ],
  [
    -84,
    114
  ],
  [
    -84,
    117
  ],
  [
    -84,
    120
  ],
  [
    -84,
    123
  ],
  [
    -84,
    126
  ],
  [
    -84,
    129
  ],
  [
    -84,
    132
  ],
  [
    -84,
    135
  ],
  [
    -84,
    138
  ],
  [
    -84,
    141
  ],
  [
    -84,
    144
  ],
  [
    -84,
    147
  ],
  [
    -84,
    150
  ],
  [
    -84,
    153
  ],
  [
    -84,
    156
  ],
  [
    -84,
    159
  ],
  [
    -84,
    162
  ],
  [
    -84,
    165
  ],
  [
    -84,
    168
  ],
  [
    -84,
    171
  ],
  [
    -84,
    174
  ],
  [
    -84,
    177
  ],
  [
    -84,
    180
  ],
  [
    -81,
    -180
  ],
  [
    -81,
    -177
  ],
  [
    -81,
    -174
  ],
  [
    -81,
    -171
  ],
  [
    -81,
    -168
  ],
  [
    -81,
    -165
  ],
  [
    -81,
    -162
  ],
  [
    -81,
    -159
  ],
  [
    -81,
    -156
  ],
  [
    -81,
    -153
  ],
  [
    -81,
    -150
  ],
  [
    -81,
    -147
  ],
  [
    -81,
    -144
  ],
  [
    -81,
    -141
  ],
  [
    -81,
    -138
  ],
  [
    -81,
    -135
  ],
  [
    -81,
    -132
  ],
  [
    -81,
    -129
  ],
  [
    -81,
    -126
  ],
  [
    -81,
    -123
  ],
  [
    -81,
    -120
  ],
  [
    -81,
    -117
  ],
  [
    -81,
    -114
  ],
  [
    -81,
    -111
  ],
  [
    -81,
    -108
  ],
  [
    -81,
    -105
  ],
  [
    -81,
    -102
  ],
  [
    -81,
    -99
  ],
  [
    -81,
    -96
  ],
  [
    -81,
    -93
  ],
  [
    -81,
    -90
  ],
  [
    -81,
    -87
  ],
  [
    -81,
    -84
  ],
  [
    -81,
    -81
  ],
  [
    -81,
    -78
  ],
  [
    -81,
    -75
  ],
  [
    -81,
    -72
  ],
  [
    -81,
    -69
  ],
  [
    -81,
    -66
  ],
  [
    -81,
    -63
  ],
  [
    -81,
    -60
  ],
  [
    -81,
    -57
  ],
  [
    -81,
    -54
  ],
  [
    -81,
    -51
  ],
  [
    -81,
    -48
  ],
  [
    -81,
    -45
  ],
  [
    -81,
    -42
  ],
  [
    -81,
    -39
  ],
  [
    -81,
    -36
  ],
  [
    -81,
    -33
  ],
  [
    -81,
    -30
  ],
  [
    -81,
    -27
  ],
  [
    -81,
    -24
  ],
  [
    -81,
    -21
  ],
  [
    -81,
    -18
  ],
  [
    -81,
    -15
  ],
  [
    -81,
    -12
  ],
  [
    -81,
    -9
  ],
  [
    -81,
    -6
  ],
  [
    -81,
    -3
  ],
  [
    -81,
    0
  ],
  [
    -81,
    3
  ],
  [
    -81,
    6
  ],
  [
    -81,
    9
  ],
  [
    -81,
    12
  ],
  [
    -81,
    15
  ],
  [
    -81,
    18
  ],
  [
    -81,
    21
  ],
  [
    -81,
    24
  ],
  [
    -81,
    27
  ],
  [
    -81,
    30
  ],
  [
    -81,
    33
  ],
  [
    -81,
    36
  ],
  [
    -81,
    39
  ],
  [
    -81,
    42
  ],
  [
    -81,
    45
  ],
  [
    -81,
    48
  ],
  [
    -81,
    51
  ],
  [
    -81,
    54
  ],
  [
    -81,
    57
  ],
  [
    -81,
    60
  ],
  [
    -81,
    63
  ],
  [
    -81,
    66
  ],
  [
    -81,
    69
  ],
  [
    -81,
    72
  ],
  [
    -81,
    75
  ],
  [
    -81,
    78
  ],
  [
    -81,
    81
  ],
  [
    -81,
    84
  ],
  [
    -81,
    87
  ],
  [
    -81,
    90
  ],
  [
    -81,
    93
  ],
  [
    -81,
    96
  ],
  [
    -81,
    99
  ],
  [
    -81,
    102
  ],
  [
    -81,
    105
  ],
  [
    -81,
    108
  ],
  [
    -81,
    111
  ],
  [
    -81,
    114
  ],
  [
    -81,
    117
  ],
  [
    -81,
    120
  ],
  [
    -81,
    123
  ],
  [
    -81,
    126
  ],
  [
    -81,
    129
  ],
  [
    -81,
    132
  ],
  [
    -81,
    135
  ],
  [
    -81,
    138
  ],
  [
    -81,
    141
  ],
  [
    -81,
    144
  ],
  [
    -81,
    147
  ],
  [
    -81,
    150
  ],
  [
    -81,
    153
  ],
  [
    -81,
    156
  ],
  [
    -81,
    159
  ],
  [
    -81,
    162
  ],
  [
    -81,
    165
  ],
  [
    -81,
    168
  ],
  [
    -81,
    171
  ],
  [
    -81,
    174
  ],
  [
    -81,
    177
  ],
  [
    -81,
    180
  ],
  [
    -78,
    -180
  ],
  [
    -78,
    -177
  ],
  [
    -78,
    -159
  ],
  [
    -78,
    -156
  ],
  [
    -78,
    -153
  ],
  [
    -78,
    -150
  ],
  [
    -78,
    -147
  ],
  [
    -78,
    -144
  ],
  [
    -78,
    -141
  ],
  [
    -78,
    -138
  ],
  [
    -78,
    -135
  ],
  [
    -78,
    -132
  ],
  [
    -78,
    -129
  ],
  [
    -78,
    -126
  ],
  [
    -78,
    -123
  ],
  [
    -78,
    -120
  ],
  [
    -78,
    -117
  ],
  [
    -78,
    -114
  ],
  [
    -78,
    -111
  ],
  [
    -78,
    -108
  ],
  [
    -78,
    -105
  ],
  [
    -78,
    -102
  ],
  [
    -78,
    -99
  ],
  [
    -78,
    -96
  ],
  [
    -78,
    -93
  ],
  [
    -78,
    -90
  ],
  [
    -78,
    -87
  ],
  [
    -78,
    -84
  ],
  [
    -78,
    -81
  ],
  [
    -78,
    -78
  ],
  [
    -78,
    -75
  ],
  [
    -78,
    -72
  ],
  [
    -78,
    -69
  ],
  [
    -78,
    -66
  ],
  [
    -78,
    -63
  ],
  [
    -78,
    -60
  ],
  [
    -78,
    -57
  ],
  [
    -78,
    -54
  ],
  [
    -78,
    -51
  ],
  [
    -78,
    -48
  ],
  [
    -78,
    -45
  ],
  [
    -78,
    -42
  ],
  [
    -78,
    -33
  ],
  [
    -78,
    -30
  ],
  [
    -78,
    -27
  ],
  [
    -78,
    -24
  ],
  [
    -78,
    -21
  ],
  [
    -78,
    -18
  ],
  [
    -78,
    -15
  ],
  [
    -78,
    -12
  ],
  [
    -78,
    -9
  ],
  [
    -78,
    -6
  ],
  [
    -78,
    -3
  ],
  [
    -78,
    0
  ],
  [
    -78,
    3
  ],
  [
    -78,
    6
  ],
  [
    -78,
    9
  ],
  [
    -78,
    12
  ],
  [
    -78,
    15
  ],
  [
    -78,
    18
  ],
  [
    -78,
    21
  ],
  [
    -78,
    24
  ],
  [
    -78,
    27
  ],
  [
    -78,
    30
  ],
  [
    -78,
    33
  ],
  [
    -78,
    36
  ],
  [
    -78,
    39
  ],
  [
    -78,
    42
  ],
  [
    -78,
    45
  ],
  [
    -78,
    48
  ],
  [
    -78,
    51
  ],
  [
    -78,
    54
  ],
  [
    -78,
    57
  ],
  [
    -78,
    60
  ],
  [
    -78,
    63
  ],
  [
    -78,
    66
  ],
  [
    -78,
    69
  ],
  [
    -78,
    72
  ],
  [
    -78,
    75
  ],
  [
    -78,
    78
  ],
  [
    -78,
    81
  ],
  [
    -78,
    84
  ],
  [
    -78,
    87
  ],
  [
    -78,
    90
  ],
  [
    -78,
    93
  ],
  [
    -78,
    96
  ],
  [
    -78,
    99
  ],
  [
    -78,
    102
  ],
  [
    -78,
    105
  ],
  [
    -78,
    108
  ],
  [
    -78,
    111
  ],
  [
    -78,
    114
  ],
  [
    -78,
    117
  ],
  [
    -78,
    120
  ],
  [
    -78,
    123
  ],
  [
    -78,
    126
  ],
  [
    -78,
    129
  ],
  [
    -78,
    132
  ],
  [
    -78,
    135
  ],
  [
    -78,
    138
  ],
  [
    -78,
    141
  ],
  [
    -78,
    144
  ],
  [
    -78,
    147
  ],
  [
    -78,
    150
  ],
  [
    -78,
    153
  ],
  [
    -78,
    156
  ],
  [
    -78,
    159
  ],
  [
    -78,
    162
  ],
  [
    -78,
    165
  ],
  [
    -78,
    168
  ],
  [
    -78,
    171
  ],
  [
    -78,
    174
  ],
  [
    -78,
    177
  ],
  [
    -78,
    180
  ],
  [
    -75,
    -180
  ],
  [
    -75,
    -135
  ],
  [
    -75,
    -132
  ],
  [
    -75,
    -129
  ],
  [
    -75,
    -126
  ],
  [
    -75,
    -123
  ],
  [
    -75,
    -120
  ],
  [
    -75,
    -117
  ],
  [
    -75,
    -114
  ],
  [
    -75,
    -111
  ],
  [
    -75,
    -105
  ],
  [
    -75,
    -99
  ],
  [
    -75,
    -96
  ],
  [
    -75,
    -93
  ],
  [
    -75,
    -90
  ],
  [
    -75,
    -87
  ],
  [
    -75,
    -84
  ],
  [
    -75,
    -81
  ],
  [
    -75,
    -78
  ],
  [
    -75,
    -75
  ],
  [
    -75,
    -72
  ],
  [
    -75,
    -69
  ],
  [
    -75,
    -66
  ],
  [
    -75,
    -63
  ],
  [
    -75,
    -24
  ],
  [
    -75,
    -21
  ],
  [
    -75,
    -18
  ],
  [
    -75,
    -15
  ],
  [
    -75,
    -12
  ],
  [
    -75,
    -9
  ],
  [
    -75,
    -6
  ],
  [
    -75,
    -3
  ],
  [
    -75,
    0
  ],
  [
    -75,
    3
  ],
  [
    -75,
    6
  ],
  [
    -75,
    9
  ],
  [
    -75,
    12
  ],
  [
    -75,
    15
  ],
  [
    -75,
    18
  ],
  [
    -75,
    21
  ],
  [
    -75,
    24
  ],
  [
    -75,
    27
  ],
  [
    -75,
    30
  ],
  [
    -75,
    33
  ],
  [
    -75,
    36
  ],
  [
    -75,
    39
  ],
  [
    -75,
    42
  ],
  [
    -75,
    45
  ],
  [
    -75,
    48
  ],
  [
    -75,
    51
  ],
  [
    -75,
    54
  ],
  [
    -75,
    57
  ],
  [
    -75,
    60
  ],
  [
    -75,
    63
  ],
  [
    -75,
    66
  ],
  [
    -75,
    69
  ],
  [
    -75,
    72
  ],
  [
    -75,
    75
  ],
  [
    -75,
    78
  ],
  [
    -75,
    81
  ],
  [
    -75,
    84
  ],
  [
    -75,
    87
  ],
  [
    -75,
    90
  ],
  [
    -75,
    93
  ],
  [
    -75,
    96
  ],
  [
    -75,
    99
  ],
  [
    -75,
    102
  ],
  [
    -75,
    105
  ],
  [
    -75,
    108
  ],
  [
    -75,
    111
  ],
  [
    -75,
    114
  ],
  [
    -75,
    117
  ],
  [
    -75,
    120
  ],
  [
    -75,
    123
  ],
  [
    -75,
    126
  ],
  [
    -75,
    129
  ],
  [
    -75,
    132
  ],
  [
    -75,
    135
  ],
  [
    -75,
    138
  ],
  [
    -75,
    141
  ],
  [
    -75,
    144
  ],
  [
    -75,
    147
  ],
  [
    -75,
    150
  ],
  [
    -75,
    153
  ],
  [
    -75,
    156
  ],
  [
    -75,
    159
  ],
  [
    -75,
    162
  ],
  [
    -75,
    180
  ],
  [
    -72,
    -180
  ],
  [
    -72,
    -102
  ],
  [
    -72,
    -99
  ],
  [
    -72,
    -96
  ],
  [
    -72,
    -75
  ],
  [
    -72,
    -72
  ],
  [
    -72,
    -69
  ],
  [
    -72,
    -66
  ],
  [
    -72,
    -63
  ],
  [
    -72,
    -12
  ],
  [
    -72,
    -9
  ],
  [
    -72,
    -6
  ],
  [
    -72,
    -3
  ],
  [
    -72,
    0
  ],
  [
    -72,
    3
  ],
  [
    -72,
    6
  ],
  [
    -72,
    9
  ],
  [
    -72,
    12
  ],
  [
    -72,
    15
  ],
  [
    -72,
    18
  ],
  [
    -72,
    21
  ],
  [
    -72,
    24
  ],
  [
    -72,
    27
  ],
  [
    -72,
    30
  ],
  [
    -72,
    33
  ],
  [
    -72,
    36
  ],
  [
    -72,
    39
  ],
  [
    -72,
    42
  ],
  [
    -72,
    45
  ],
  [
    -72,
    48
  ],
  [
    -72,
    51
  ],
  [
    -72,
    54
  ],
  [
    -72,
    57
  ],
  [
    -72,
    60
  ],
  [
    -72,
    63
  ],
  [
    -72,
    66
  ],
  [
    -72,
    69
  ],
  [
    -72,
    72
  ],
  [
    -72,
    75
  ],
  [
    -72,
    78
  ],
  [
    -72,
    81
  ],
  [
    -72,
    84
  ],
  [
    -72,
    87
  ],
  [
    -72,
    90
  ],
  [
    -72,
    93
  ],
  [
    -72,
    96
  ],
  [
    -72,
    99
  ],
  [
    -72,
    102
  ],
  [
    -72,
    105
  ],
  [
    -72,
    108
  ],
  [
    -72,
    111
  ],
  [
    -72,
    114
  ],
  [
    -72,
    117
  ],
  [
    -72,
    120
  ],
  [
    -72,
    123
  ],
  [
    -72,
    126
  ],
  [
    -72,
    129
  ],
  [
    -72,
    132
  ],
  [
    -72,
    135
  ],
  [
    -72,
    138
  ],
  [
    -72,
    141
  ],
  [
    -72,
    144
  ],
  [
    -72,
    147
  ],
  [
    -72,
    150
  ],
  [
    -72,
    153
  ],
  [
    -72,
    156
  ],
  [
    -72,
    159
  ],
  [
    -72,
    162
  ],
  [
    -72,
    165
  ],
  [
    -72,
    168
  ],
  [
    -72,
    180
  ],
  [
    -69,
    -180
  ],
  [
    -69,
    -72
  ],
  [
    -69,
    -66
  ],
  [
    -69,
    -63
  ],
  [
    -69,
    33
  ],
  [
    -69,
    42
  ],
  [
    -69,
    45
  ],
  [
    -69,
    48
  ],
  [
    -69,
    51
  ],
  [
    -69,
    54
  ],
  [
    -69,
    57
  ],
  [
    -69,
    60
  ],
  [
    -69,
    63
  ],
  [
    -69,
    66
  ],
  [
    -69,
    69
  ],
  [
    -69,
    72
  ],
  [
    -69,
    78
  ],
  [
    -69,
    81
  ],
  [
    -69,
    84
  ],
  [
    -69,
    87
  ],
  [
    -69,
    90
  ],
  [
    -69,
    93
  ],
  [
    -69,
    96
  ],
  [
    -69,
    99
  ],
  [
    -69,
    102
  ],
  [
    -69,
    105
  ],
  [
    -69,
    108
  ],
  [
    -69,
    111
  ],
  [
    -69,
    114
  ],
  [
    -69,
    117
  ],
  [
    -69,
    120
  ],
  [
    -69,
    123
  ],
  [
    -69,
    126
  ],
  [
    -69,
    129
  ],
  [
    -69,
    132
  ],
  [
    -69,
    135
  ],
  [
    -69,
    138
  ],
  [
    -69,
    141
  ],
  [
    -69,
    144
  ],
  [
    -69,
    147
  ],
  [
    -69,
    150
  ],
  [
    -69,
    153
  ],
  [
    -69,
    156
  ],
  [
    -69,
    180
  ],
  [
    -66,
    -180
  ],
  [
    -66,
    -63
  ],
  [
    -66,
    54
  ],
  [
    -66,
    96
  ],
  [
    -66,
    99
  ],
  [
    -66,
    102
  ],
  [
    -66,
    180
  ],
  [
    -63,
    -180
  ],
  [
    -63,
    180
  ],
  [
    -60,
    -180
  ],
  [
    -60,
    180
  ],
  [
    -57,
    -180
  ],
  [
    -57,
    180
  ],
  [
    -54,
    -180
  ],
  [
    -54,
    -72
  ],
  [
    -54,
    -69
  ],
  [
    -54,
    180
  ],
  [
    -51,
    -180
  ],
  [
    -51,
    -75
  ],
  [
    -51,
    -72
  ],
  [
    -51,
    180
  ],
  [
    -48,
    -180
  ],
  [
    -48,
    -75
  ],
  [
    -48,
    -72
  ],
  [
    -48,
    -69
  ],
  [
    -48,
    -66
  ],
  [
    -48,
    180
  ],
  [
    -45,
    -180
  ],
  [
    -45,
    -72
  ],
  [
    -45,
    -69
  ],
  [
    -45,
    -66
  ],
  [
    -45,
    168
  ],
  [
    -45,
    171
  ],
  [
    -45,
    180
  ],
  [
    -42,
    -180
  ],
  [
    -42,
    -72
  ],
  [
    -42,
    -69
  ],
  [
    -42,
    -66
  ],
  [
    -42,
    147
  ],
  [
    -42,
    174
  ],
  [
    -42,
    180
  ],
  [
    -39,
    -180
  ],
  [
    -39,
    -72
  ],
  [
    -39,
    -69
  ],
  [
    -39,
    -66
  ],
  [
    -39,
    -63
  ],
  [
    -39,
    177
  ],
  [
    -39,
    180
  ],
  [
    -36,
    -180
  ],
  [
    -36,
    -72
  ],
  [
    -36,
    -69
  ],
  [
    -36,
    -66
  ],
  [
    -36,
    -63
  ],
  [
    -36,
    -60
  ],
  [
    -36,
    -57
  ],
  [
    -36,
    141
  ],
  [
    -36,
    144
  ],
  [
    -36,
    147
  ],
  [
    -36,
    150
  ],
  [
    -36,
    174
  ],
  [
    -36,
    180
  ],
  [
    -33,
    -180
  ],
  [
    -33,
    -69
  ],
  [
    -33,
    -66
  ],
  [
    -33,
    -63
  ],
  [
    -33,
    -60
  ],
  [
    -33,
    -57
  ],
  [
    -33,
    -54
  ],
  [
    -33,
    18
  ],
  [
    -33,
    21
  ],
  [
    -33,
    24
  ],
  [
    -33,
    27
  ],
  [
    -33,
    117
  ],
  [
    -33,
    120
  ],
  [
    -33,
    123
  ],
  [
    -33,
    135
  ],
  [
    -33,
    138
  ],
  [
    -33,
    141
  ],
  [
    -33,
    144
  ],
  [
    -33,
    147
  ],
  [
    -33,
    150
  ],
  [
    -33,
    180
  ],
  [
    -30,
    -180
  ],
  [
    -30,
    -69
  ],
  [
    -30,
    -66
  ],
  [
    -30,
    -63
  ],
  [
    -30,
    -60
  ],
  [
    -30,
    -57
  ],
  [
    -30,
    -54
  ],
  [
    -30,
    -51
  ],
  [
    -30,
    18
  ],
  [
    -30,
    21
  ],
  [
    -30,
    24
  ],
  [
    -30,
    27
  ],
  [
    -30,
    30
  ],
  [
    -30,
    117
  ],
  [
    -30,
    120
  ],
  [
    -30,
    123
  ],
  [
    -30,
    126
  ],
  [
    -30,
    129
  ],
  [
    -30,
    132
  ],
  [
    -30,
    135
  ],
  [
    -30,
    138
  ],
  [
    -30,
    141
  ],
  [
    -30,
    144
  ],
  [
    -30,
    147
  ],
  [
    -30,
    150
  ],
  [
    -30,
    153
  ],
  [
    -30,
    180
  ],
  [
    -27,
    -180
  ],
  [
    -27,
    -69
  ],
  [
    -27,
    -66
  ],
  [
    -27,
    -63
  ],
  [
    -27,
    -60
  ],
  [
    -27,
    -57
  ],
  [
    -27,
    -54
  ],
  [
    -27,
    -51
  ],
  [
    -27,
    18
  ],
  [
    -27,
    21
  ],
  [
    -27,
    24
  ],
  [
    -27,
    27
  ],
  [
    -27,
    30
  ],
  [
    -27,
    114
  ],
  [
    -27,
    117
  ],
  [
    -27,
    120
  ],
  [
    -27,
    123
  ],
  [
    -27,
    126
  ],
  [
    -27,
    129
  ],
  [
    -27,
    132
  ],
  [
    -27,
    135
  ],
  [
    -27,
    138
  ],
  [
    -27,
    141
  ],
  [
    -27,
    144
  ],
  [
    -27,
    147
  ],
  [
    -27,
    150
  ],
  [
    -27,
    153
  ],
  [
    -27,
    180
  ],
  [
    -24,
    -180
  ],
  [
    -24,
    -69
  ],
  [
    -24,
    -66
  ],
  [
    -24,
    -63
  ],
  [
    -24,
    -60
  ],
  [
    -24,
    -57
  ],
  [
    -24,
    -54
  ],
  [
    -24,
    -51
  ],
  [
    -24,
    -48
  ],
  [
    -24,
    15
  ],
  [
    -24,
    18
  ],
  [
    -24,
    21
  ],
  [
    -24,
    24
  ],
  [
    -24,
    27
  ],
  [
    -24,
    30
  ],
  [
    -24,
    33
  ],
  [
    -24,
    45
  ],
  [
    -24,
    114
  ],
  [
    -24,
    117
  ],
  [
    -24,
    120
  ],
  [
    -24,
    123
  ],
  [
    -24,
    126
  ],
  [
    -24,
    129
  ],
  [
    -24,
    132
  ],
  [
    -24,
    135
  ],
  [
    -24,
    138
  ],
  [
    -24,
    141
  ],
  [
    -24,
    144
  ],
  [
    -24,
    147
  ],
  [
    -24,
    150
  ],
  [
    -24,
    180
  ],
  [
    -21,
    -180
  ],
  [
    -21,
    -69
  ],
  [
    -21,
    -66
  ],
  [
    -21,
    -63
  ],
  [
    -21,
    -60
  ],
  [
    -21,
    -57
  ],
  [
    -21,
    -54
  ],
  [
    -21,
    -51
  ],
  [
    -21,
    -48
  ],
  [
    -21,
    -45
  ],
  [
    -21,
    -42
  ],
  [
    -21,
    15
  ],
  [
    -21,
    18
  ],
  [
    -21,
    21
  ],
  [
    -21,
    24
  ],
  [
    -21,
    27
  ],
  [
    -21,
    30
  ],
  [
    -21,
    33
  ],
  [
    -21,
    45
  ],
  [
    -21,
    48
  ],
  [
    -21,
    117
  ],
  [
    -21,
    120
  ],
  [
    -21,
    123
  ],
  [
    -21,
    126
  ],
  [
    -21,
    129
  ],
  [
    -21,
    132
  ],
  [
    -21,
    135
  ],
  [
    -21,
    138
  ],
  [
    -21,
    141
  ],
  [
    -21,
    144
  ],
  [
    -21,
    147
  ],
  [
    -21,
    165
  ],
  [
    -21,
    180
  ],
  [
    -18,
    -180
  ],
  [
    -18,
    -69
  ],
  [
    -18,
    -66
  ],
  [
    -18,
    -63
  ],
  [
    -18,
    -60
  ],
  [
    -18,
    -57
  ],
  [
    -18,
    -54
  ],
  [
    -18,
    -51
  ],
  [
    -18,
    -48
  ],
  [
    -18,
    -45
  ],
  [
    -18,
    -42
  ],
  [
    -18,
    12
  ],
  [
    -18,
    15
  ],
  [
    -18,
    18
  ],
  [
    -18,
    21
  ],
  [
    -18,
    24
  ],
  [
    -18,
    27
  ],
  [
    -18,
    30
  ],
  [
    -18,
    33
  ],
  [
    -18,
    36
  ],
  [
    -18,
    45
  ],
  [
    -18,
    48
  ],
  [
    -18,
    123
  ],
  [
    -18,
    126
  ],
  [
    -18,
    129
  ],
  [
    -18,
    132
  ],
  [
    -18,
    135
  ],
  [
    -18,
    138
  ],
  [
    -18,
    141
  ],
  [
    -18,
    144
  ],
  [
    -18,
    180
  ],
  [
    -15,
    -180
  ],
  [
    -15,
    -75
  ],
  [
    -15,
    -72
  ],
  [
    -15,
    -69
  ],
  [
    -15,
    -66
  ],
  [
    -15,
    -63
  ],
  [
    -15,
    -60
  ],
  [
    -15,
    -57
  ],
  [
    -15,
    -54
  ],
  [
    -15,
    -51
  ],
  [
    -15,
    -48
  ],
  [
    -15,
    -45
  ],
  [
    -15,
    -42
  ],
  [
    -15,
    -39
  ],
  [
    -15,
    15
  ],
  [
    -15,
    18
  ],
  [
    -15,
    21
  ],
  [
    -15,
    24
  ],
  [
    -15,
    27
  ],
  [
    -15,
    30
  ],
  [
    -15,
    33
  ],
  [
    -15,
    36
  ],
  [
    -15,
    39
  ],
  [
    -15,
    48
  ],
  [
    -15,
    126
  ],
  [
    -15,
    129
  ],
  [
    -15,
    132
  ],
  [
    -15,
    135
  ],
  [
    -15,
    144
  ],
  [
    -15,
    180
  ],
  [
    -12,
    -180
  ],
  [
    -12,
    -75
  ],
  [
    -12,
    -72
  ],
  [
    -12,
    -69
  ],
  [
    -12,
    -66
  ],
  [
    -12,
    -63
  ],
  [
    -12,
    -60
  ],
  [
    -12,
    -57
  ],
  [
    -12,
    -54
  ],
  [
    -12,
    -51
  ],
  [
    -12,
    -48
  ],
  [
    -12,
    -45
  ],
  [
    -12,
    -42
  ],
  [
    -12,
    -39
  ],
  [
    -12,
    15
  ],
  [
    -12,
    18
  ],
  [
    -12,
    21
  ],
  [
    -12,
    24
  ],
  [
    -12,
    27
  ],
  [
    -12,
    30
  ],
  [
    -12,
    33
  ],
  [
    -12,
    36
  ],
  [
    -12,
    39
  ],
  [
    -12,
    180
  ],
  [
    -9,
    -180
  ],
  [
    -9,
    -78
  ],
  [
    -9,
    -75
  ],
  [
    -9,
    -72
  ],
  [
    -9,
    -69
  ],
  [
    -9,
    -66
  ],
  [
    -9,
    -63
  ],
  [
    -9,
    -60
  ],
  [
    -9,
    -57
  ],
  [
    -9,
    -54
  ],
  [
    -9,
    -51
  ],
  [
    -9,
    -48
  ],
  [
    -9,
    -45
  ],
  [
    -9,
    -42
  ],
  [
    -9,
    -39
  ],
  [
    -9,
    -36
  ],
  [
    -9,
    15
  ],
  [
    -9,
    18
  ],
  [
    -9,
    21
  ],
  [
    -9,
    24
  ],
  [
    -9,
    27
  ],
  [
    -9,
    30
  ],
  [
    -9,
    33
  ],
  [
    -9,
    36
  ],
  [
    -9,
    39
  ],
  [
    -9,
    117
  ],
  [
    -9,
    126
  ],
  [
    -9,
    141
  ],
  [
    -9,
    147
  ],
  [
    -9,
    180
  ],
  [
    -6,
    -180
  ],
  [
    -6,
    -81
  ],
  [
    -6,
    -78
  ],
  [
    -6,
    -75
  ],
  [
    -6,
    -72
  ],
  [
    -6,
    -69
  ],
  [
    -6,
    -66
  ],
  [
    -6,
    -63
  ],
  [
    -6,
    -60
  ],
  [
    -6,
    -57
  ],
  [
    -6,
    -54
  ],
  [
    -6,
    -51
  ],
  [
    -6,
    -48
  ],
  [
    -6,
    -45
  ],
  [
    -6,
    -42
  ],
  [
    -6,
    -39
  ],
  [
    -6,
    -36
  ],
  [
    -6,
    15
  ],
  [
    -6,
    18
  ],
  [
    -6,
    21
  ],
  [
    -6,
    24
  ],
  [
    -6,
    27
  ],
  [
    -6,
    30
  ],
  [
    -6,
    33
  ],
  [
    -6,
    36
  ],
  [
    -6,
    141
  ],
  [
    -6,
    144
  ],
  [
    -6,
    147
  ],
  [
    -6,
    150
  ],
  [
    -6,
    180
  ],
  [
    -3,
    -180
  ],
  [
    -3,
    -78
  ],
  [
    -3,
    -75
  ],
  [
    -3,
    -72
  ],
  [
    -3,
    -69
  ],
  [
    -3,
    -66
  ],
  [
    -3,
    -63
  ],
  [
    -3,
    -60
  ],
  [
    -3,
    -57
  ],
  [
    -3,
    -54
  ],
  [
    -3,
    -51
  ],
  [
    -3,
    -48
  ],
  [
    -3,
    -45
  ],
  [
    -3,
    -42
  ],
  [
    -3,
    12
  ],
  [
    -3,
    15
  ],
  [
    -3,
    18
  ],
  [
    -3,
    21
  ],
  [
    -3,
    24
  ],
  [
    -3,
    27
  ],
  [
    -3,
    30
  ],
  [
    -3,
    33
  ],
  [
    -3,
    36
  ],
  [
    -3,
    39
  ],
  [
    -3,
    102
  ],
  [
    -3,
    105
  ],
  [
    -3,
    108
  ],
  [
    -3,
    111
  ],
  [
    -3,
    114
  ],
  [
    -3,
    120
  ],
  [
    -3,
    129
  ],
  [
    -3,
    138
  ],
  [
    -3,
    141
  ],
  [
    -3,
    180
  ],
  [
    0,
    -180
  ],
  [
    0,
    -78
  ],
  [
    0,
    -75
  ],
  [
    0,
    -72
  ],
  [
    0,
    -69
  ],
  [
    0,
    -66
  ],
  [
    0,
    -63
  ],
  [
    0,
    -60
  ],
  [
    0,
    -57
  ],
  [
    0,
    -54
  ],
  [
    0,
    -51
  ],
  [
    0,
    12
  ],
  [
    0,
    15
  ],
  [
    0,
    18
  ],
  [
    0,
    21
  ],
  [
    0,
    24
  ],
  [
    0,
    27
  ],
  [
    0,
    30
  ],
  [
    0,
    33
  ],
  [
    0,
    36
  ],
  [
    0,
    39
  ],
  [
    0,
    42
  ],
  [
    0,
    102
  ],
  [
    0,
    111
  ],
  [
    0,
    114
  ],
  [
    0,
    117
  ],
  [
    0,
    120
  ],
  [
    0,
    180
  ],
  [
    3,
    -180
  ],
  [
    3,
    -75
  ],
  [
    3,
    -72
  ],
  [
    3,
    -69
  ],
  [
    3,
    -66
  ],
  [
    3,
    -63
  ],
  [
    3,
    -60
  ],
  [
    3,
    -57
  ],
  [
    3,
    -54
  ],
  [
    3,
    -51
  ],
  [
    3,
    12
  ],
  [
    3,
    15
  ],
  [
    3,
    18
  ],
  [
    3,
    21
  ],
  [
    3,
    24
  ],
  [
    3,
    27
  ],
  [
    3,
    30
  ],
  [
    3,
    33
  ],
  [
    3,
    36
  ],
  [
    3,
    39
  ],
  [
    3,
    42
  ],
  [
    3,
    45
  ],
  [
    3,
    99
  ],
  [
    3,
    102
  ],
  [
    3,
    114
  ],
  [
    3,
    117
  ],
  [
    3,
    180
  ],
  [
    6,
    -180
  ],
  [
    6,
    -75
  ],
  [
    6,
    -72
  ],
  [
    6,
    -69
  ],
  [
    6,
    -66
  ],
  [
    6,
    -63
  ],
  [
    6,
    -60
  ],
  [
    6,
    -9
  ],
  [
    6,
    -6
  ],
  [
    6,
    -3
  ],
  [
    6,
    0
  ],
  [
    6,
    6
  ],
  [
    6,
    9
  ],
  [
    6,
    12
  ],
  [
    6,
    15
  ],
  [
    6,
    18
  ],
  [
    6,
    21
  ],
  [
    6,
    24
  ],
  [
    6,
    27
  ],
  [
    6,
    30
  ],
  [
    6,
    33
  ],
  [
    6,
    36
  ],
  [
    6,
    39
  ],
  [
    6,
    42
  ],
  [
    6,
    45
  ],
  [
    6,
    48
  ],
  [
    6,
    102
  ],
  [
    6,
    117
  ],
  [
    6,
    180
  ],
  [
    9,
    -180
  ],
  [
    9,
    -78
  ],
  [
    9,
    -75
  ],
  [
    9,
    -72
  ],
  [
    9,
    -69
  ],
  [
    9,
    -66
  ],
  [
    9,
    -63
  ],
  [
    9,
    -12
  ],
  [
    9,
    -9
  ],
  [
    9,
    -6
  ],
  [
    9,
    -3
  ],
  [
    9,
    0
  ],
  [
    9,
    3
  ],
  [
    9,
    6
  ],
  [
    9,
    9
  ],
  [
    9,
    12
  ],
  [
    9,
    15
  ],
  [
    9,
    18
  ],
  [
    9,
    21
  ],
  [
    9,
    24
  ],
  [
    9,
    27
  ],
  [
    9,
    30
  ],
  [
    9,
    33
  ],
  [
    9,
    36
  ],
  [
    9,
    39
  ],
  [
    9,
    42
  ],
  [
    9,
    45
  ],
  [
    9,
    48
  ],
  [
    9,
    78
  ],
  [
    9,
    99
  ],
  [
    9,
    105
  ],
  [
    9,
    126
  ],
  [
    9,
    180
  ],
  [
    12,
    -180
  ],
  [
    12,
    -84
  ],
  [
    12,
    -72
  ],
  [
    12,
    -15
  ],
  [
    12,
    -12
  ],
  [
    12,
    -9
  ],
  [
    12,
    -6
  ],
  [
    12,
    -3
  ],
  [
    12,
    0
  ],
  [
    12,
    3
  ],
  [
    12,
    6
  ],
  [
    12,
    9
  ],
  [
    12,
    12
  ],
  [
    12,
    15
  ],
  [
    12,
    18
  ],
  [
    12,
    21
  ],
  [
    12,
    24
  ],
  [
    12,
    27
  ],
  [
    12,
    30
  ],
  [
    12,
    33
  ],
  [
    12,
    36
  ],
  [
    12,
    39
  ],
  [
    12,
    42
  ],
  [
    12,
    78
  ],
  [
    12,
    93
  ],
  [
    12,
    99
  ],
  [
    12,
    105
  ],
  [
    12,
    108
  ],
  [
    12,
    180
  ],
  [
    15,
    -180
  ],
  [
    15,
    -90
  ],
  [
    15,
    -87
  ],
  [
    15,
    -84
  ],
  [
    15,
    -15
  ],
  [
    15,
    -12
  ],
  [
    15,
    -9
  ],
  [
    15,
    -6
  ],
  [
    15,
    -3
  ],
  [
    15,
    0
  ],
  [
    15,
    3
  ],
  [
    15,
    6
  ],
  [
    15,
    9
  ],
  [
    15,
    12
  ],
  [
    15,
    15
  ],
  [
    15,
    18
  ],
  [
    15,
    21
  ],
  [
    15,
    24
  ],
  [
    15,
    27
  ],
  [
    15,
    30
  ],
  [
    15,
    33
  ],
  [
    15,
    36
  ],
  [
    15,
    39
  ],
  [
    15,
    45
  ],
  [
    15,
    48
  ],
  [
    15,
    75
  ],
  [
    15,
    78
  ],
  [
    15,
    99
  ],
  [
    15,
    102
  ],
  [
    15,
    105
  ],
  [
    15,
    108
  ],
  [
    15,
    180
  ],
  [
    18,
    -180
  ],
  [
    18,
    -102
  ],
  [
    18,
    -99
  ],
  [
    18,
    -96
  ],
  [
    18,
    -93
  ],
  [
    18,
    -90
  ],
  [
    18,
    -66
  ],
  [
    18,
    -15
  ],
  [
    18,
    -12
  ],
  [
    18,
    -9
  ],
  [
    18,
    -6
  ],
  [
    18,
    -3
  ],
  [
    18,
    0
  ],
  [
    18,
    3
  ],
  [
    18,
    6
  ],
  [
    18,
    9
  ],
  [
    18,
    12
  ],
  [
    18,
    15
  ],
  [
    18,
    18
  ],
  [
    18,
    21
  ],
  [
    18,
    24
  ],
  [
    18,
    27
  ],
  [
    18,
    30
  ],
  [
    18,
    33
  ],
  [
    18,
    36
  ],
  [
    18,
    42
  ],
  [
    18,
    45
  ],
  [
    18,
    48
  ],
  [
    18,
    51
  ],
  [
    18,
    54
  ],
  [
    18,
    75
  ],
  [
    18,
    78
  ],
  [
    18,
    81
  ],
  [
    18,
    96
  ],
  [
    18,
    99
  ],
  [
    18,
    102
  ],
  [
    18,
    105
  ],
  [
    18,
    180
  ],
  [
    21,
    -180
  ],
  [
    21,
    -105
  ],
  [
    21,
    -102
  ],
  [
    21,
    -99
  ],
  [
    21,
    -90
  ],
  [
    21,
    -87
  ],
  [
    21,
    -78
  ],
  [
    21,
    -15
  ],
  [
    21,
    -12
  ],
  [
    21,
    -9
  ],
  [
    21,
    -6
  ],
  [
    21,
    -3
  ],
  [
    21,
    0
  ],
  [
    21,
    3
  ],
  [
    21,
    6
  ],
  [
    21,
    9
  ],
  [
    21,
    12
  ],
  [
    21,
    15
  ],
  [
    21,
    18
  ],
  [
    21,
    21
  ],
  [
    21,
    24
  ],
  [
    21,
    27
  ],
  [
    21,
    30
  ],
  [
    21,
    33
  ],
  [
    21,
    36
  ],
  [
    21,
    42
  ],
  [
    21,
    45
  ],
  [
    21,
    48
  ],
  [
    21,
    51
  ],
  [
    21,
    54
  ],
  [
    21,
    57
  ],
  [
    21,
    75
  ],
  [
    21,
    78
  ],
  [
    21,
    81
  ],
  [
    21,
    84
  ],
  [
    21,
    93
  ],
  [
    21,
    96
  ],
  [
    21,
    99
  ],
  [
    21,
    102
  ],
  [
    21,
    105
  ],
  [
    21,
    180
  ],
  [
    24,
    -180
  ],
  [
    24,
    -105
  ],
  [
    24,
    -102
  ],
  [
    24,
    -99
  ],
  [
    24,
    -15
  ],
  [
    24,
    -12
  ],
  [
    24,
    -9
  ],
  [
    24,
    -6
  ],
  [
    24,
    -3
  ],
  [
    24,
    0
  ],
  [
    24,
    3
  ],
  [
    24,
    6
  ],
  [
    24,
    9
  ],
  [
    24,
    12
  ],
  [
    24,
    15
  ],
  [
    24,
    18
  ],
  [
    24,
    21
  ],
  [
    24,
    24
  ],
  [
    24,
    27
  ],
  [
    24,
    30
  ],
  [
    24,
    33
  ],
  [
    24,
    39
  ],
  [
    24,
    42
  ],
  [
    24,
    45
  ],
  [
    24,
    48
  ],
  [
    24,
    51
  ],
  [
    24,
    54
  ],
  [
    24,
    57
  ],
  [
    24,
    72
  ],
  [
    24,
    75
  ],
  [
    24,
    78
  ],
  [
    24,
    81
  ],
  [
    24,
    84
  ],
  [
    24,
    87
  ],
  [
    24,
    90
  ],
  [
    24,
    93
  ],
  [
    24,
    96
  ],
  [
    24,
    99
  ],
  [
    24,
    102
  ],
  [
    24,
    105
  ],
  [
    24,
    108
  ],
  [
    24,
    111
  ],
  [
    24,
    114
  ],
  [
    24,
    117
  ],
  [
    24,
    180
  ],
  [
    27,
    -180
  ],
  [
    27,
    -114
  ],
  [
    27,
    -108
  ],
  [
    27,
    -105
  ],
  [
    27,
    -102
  ],
  [
    27,
    -99
  ],
  [
    27,
    -81
  ],
  [
    27,
    -12
  ],
  [
    27,
    -9
  ],
  [
    27,
    -6
  ],
  [
    27,
    -3
  ],
  [
    27,
    0
  ],
  [
    27,
    3
  ],
  [
    27,
    6
  ],
  [
    27,
    9
  ],
  [
    27,
    12
  ],
  [
    27,
    15
  ],
  [
    27,
    18
  ],
  [
    27,
    21
  ],
  [
    27,
    24
  ],
  [
    27,
    27
  ],
  [
    27,
    30
  ],
  [
    27,
    33
  ],
  [
    27,
    36
  ],
  [
    27,
    39
  ],
  [
    27,
    42
  ],
  [
    27,
    45
  ],
  [
    27,
    48
  ],
  [
    27,
    54
  ],
  [
    27,
    57
  ],
  [
    27,
    60
  ],
  [
    27,
    63
  ],
  [
    27,
    66
  ],
  [
    27,
    69
  ],
  [
    27,
    72
  ],
  [
    27,
    75
  ],
  [
    27,
    78
  ],
  [
    27,
    81
  ],
  [
    27,
    84
  ],
  [
    27,
    87
  ],
  [
    27,
    90
  ],
  [
    27,
    93
  ],
  [
    27,
    96
  ],
  [
    27,
    99
  ],
  [
    27,
    102
  ],
  [
    27,
    105
  ],
  [
    27,
    108
  ],
  [
    27,
    111
  ],
  [
    27,
    114
  ],
  [
    27,
    117
  ],
  [
    27,
    120
  ],
  [
    27,
    180
  ],
  [
    30,
    -180
  ],
  [
    30,
    -111
  ],
  [
    30,
    -108
  ],
  [
    30,
    -105
  ],
  [
    30,
    -102
  ],
  [
    30,
    -99
  ],
  [
    30,
    -96
  ],
  [
    30,
    -93
  ],
  [
    30,
    -90
  ],
  [
    30,
    -9
  ],
  [
    30,
    -6
  ],
  [
    30,
    -3
  ],
  [
    30,
    0
  ],
  [
    30,
    3
  ],
  [
    30,
    6
  ],
  [
    30,
    9
  ],
  [
    30,
    12
  ],
  [
    30,
    15
  ],
  [
    30,
    18
  ],
  [
    30,
    21
  ],
  [
    30,
    24
  ],
  [
    30,
    27
  ],
  [
    30,
    30
  ],
  [
    30,
    33
  ],
  [
    30,
    36
  ],
  [
    30,
    39
  ],
  [
    30,
    42
  ],
  [
    30,
    45
  ],
  [
    30,
    51
  ],
  [
    30,
    54
  ],
  [
    30,
    57
  ],
  [
    30,
    60
  ],
  [
    30,
    63
  ],
  [
    30,
    66
  ],
  [
    30,
    69
  ],
  [
    30,
    72
  ],
  [
    30,
    75
  ],
  [
    30,
    78
  ],
  [
    30,
    81
  ],
  [
    30,
    84
  ],
  [
    30,
    87
  ],
  [
    30,
    90
  ],
  [
    30,
    93
  ],
  [
    30,
    96
  ],
  [
    30,
    99
  ],
  [
    30,
    102
  ],
  [
    30,
    105
  ],
  [
    30,
    108
  ],
  [
    30,
    111
  ],
  [
    30,
    114
  ],
  [
    30,
    117
  ],
  [
    30,
    120
  ],
  [
    30,
    180
  ],
  [
    33,
    -180
  ],
  [
    33,
    -117
  ],
  [
    33,
    -114
  ],
  [
    33,
    -111
  ],
  [
    33,
    -108
  ],
  [
    33,
    -105
  ],
  [
    33,
    -102
  ],
  [
    33,
    -99
  ],
  [
    33,
    -96
  ],
  [
    33,
    -93
  ],
  [
    33,
    -90
  ],
  [
    33,
    -87
  ],
  [
    33,
    -84
  ],
  [
    33,
    -81
  ],
  [
    33,
    -6
  ],
  [
    33,
    -3
  ],
  [
    33,
    0
  ],
  [
    33,
    3
  ],
  [
    33,
    6
  ],
  [
    33,
    9
  ],
  [
    33,
    36
  ],
  [
    33,
    39
  ],
  [
    33,
    42
  ],
  [
    33,
    45
  ],
  [
    33,
    48
  ],
  [
    33,
    51
  ],
  [
    33,
    54
  ],
  [
    33,
    57
  ],
  [
    33,
    60
  ],
  [
    33,
    63
  ],
  [
    33,
    66
  ],
  [
    33,
    69
  ],
  [
    33,
    72
  ],
  [
    33,
    75
  ],
  [
    33,
    78
  ],
  [
    33,
    81
  ],
  [
    33,
    84
  ],
  [
    33,
    87
  ],
  [
    33,
    90
  ],
  [
    33,
    93
  ],
  [
    33,
    96
  ],
  [
    33,
    99
  ],
  [
    33,
    102
  ],
  [
    33,
    105
  ],
  [
    33,
    108
  ],
  [
    33,
    111
  ],
  [
    33,
    114
  ],
  [
    33,
    117
  ],
  [
    33,
    120
  ],
  [
    33,
    180
  ],
  [
    36,
    -180
  ],
  [
    36,
    -120
  ],
  [
    36,
    -117
  ],
  [
    36,
    -114
  ],
  [
    36,
    -111
  ],
  [
    36,
    -108
  ],
  [
    36,
    -105
  ],
  [
    36,
    -102
  ],
  [
    36,
    -99
  ],
  [
    36,
    -96
  ],
  [
    36,
    -93
  ],
  [
    36,
    -90
  ],
  [
    36,
    -87
  ],
  [
    36,
    -84
  ],
  [
    36,
    -81
  ],
  [
    36,
    -78
  ],
  [
    36,
    3
  ],
  [
    36,
    6
  ],
  [
    36,
    9
  ],
  [
    36,
    36
  ],
  [
    36,
    39
  ],
  [
    36,
    42
  ],
  [
    36,
    45
  ],
  [
    36,
    48
  ],
  [
    36,
    51
  ],
  [
    36,
    54
  ],
  [
    36,
    57
  ],
  [
    36,
    60
  ],
  [
    36,
    63
  ],
  [
    36,
    66
  ],
  [
    36,
    69
  ],
  [
    36,
    72
  ],
  [
    36,
    75
  ],
  [
    36,
    78
  ],
  [
    36,
    81
  ],
  [
    36,
    84
  ],
  [
    36,
    87
  ],
  [
    36,
    90
  ],
  [
    36,
    93
  ],
  [
    36,
    96
  ],
  [
    36,
    99
  ],
  [
    36,
    102
  ],
  [
    36,
    105
  ],
  [
    36,
    108
  ],
  [
    36,
    111
  ],
  [
    36,
    114
  ],
  [
    36,
    117
  ],
  [
    36,
    120
  ],
  [
    36,
    129
  ],
  [
    36,
    138
  ],
  [
    36,
    180
  ],
  [
    39,
    -180
  ],
  [
    39,
    -123
  ],
  [
    39,
    -120
  ],
  [
    39,
    -117
  ],
  [
    39,
    -114
  ],
  [
    39,
    -111
  ],
  [
    39,
    -108
  ],
  [
    39,
    -105
  ],
  [
    39,
    -102
  ],
  [
    39,
    -99
  ],
  [
    39,
    -96
  ],
  [
    39,
    -93
  ],
  [
    39,
    -90
  ],
  [
    39,
    -87
  ],
  [
    39,
    -84
  ],
  [
    39,
    -81
  ],
  [
    39,
    -78
  ],
  [
    39,
    -9
  ],
  [
    39,
    -6
  ],
  [
    39,
    -3
  ],
  [
    39,
    9
  ],
  [
    39,
    27
  ],
  [
    39,
    30
  ],
  [
    39,
    33
  ],
  [
    39,
    36
  ],
  [
    39,
    39
  ],
  [
    39,
    42
  ],
  [
    39,
    45
  ],
  [
    39,
    48
  ],
  [
    39,
    54
  ],
  [
    39,
    57
  ],
  [
    39,
    60
  ],
  [
    39,
    63
  ],
  [
    39,
    66
  ],
  [
    39,
    69
  ],
  [
    39,
    72
  ],
  [
    39,
    75
  ],
  [
    39,
    78
  ],
  [
    39,
    81
  ],
  [
    39,
    84
  ],
  [
    39,
    87
  ],
  [
    39,
    90
  ],
  [
    39,
    93
  ],
  [
    39,
    96
  ],
  [
    39,
    99
  ],
  [
    39,
    102
  ],
  [
    39,
    105
  ],
  [
    39,
    108
  ],
  [
    39,
    111
  ],
  [
    39,
    114
  ],
  [
    39,
    117
  ],
  [
    39,
    126
  ],
  [
    39,
    141
  ],
  [
    39,
    180
  ],
  [
    42,
    -180
  ],
  [
    42,
    -123
  ],
  [
    42,
    -120
  ],
  [
    42,
    -117
  ],
  [
    42,
    -114
  ],
  [
    42,
    -111
  ],
  [
    42,
    -108
  ],
  [
    42,
    -105
  ],
  [
    42,
    -102
  ],
  [
    42,
    -99
  ],
  [
    42,
    -96
  ],
  [
    42,
    -93
  ],
  [
    42,
    -90
  ],
  [
    42,
    -87
  ],
  [
    42,
    -84
  ],
  [
    42,
    -81
  ],
  [
    42,
    -78
  ],
  [
    42,
    -75
  ],
  [
    42,
    -72
  ],
  [
    42,
    -6
  ],
  [
    42,
    -3
  ],
  [
    42,
    0
  ],
  [
    42,
    3
  ],
  [
    42,
    9
  ],
  [
    42,
    12
  ],
  [
    42,
    15
  ],
  [
    42,
    21
  ],
  [
    42,
    24
  ],
  [
    42,
    27
  ],
  [
    42,
    42
  ],
  [
    42,
    45
  ],
  [
    42,
    48
  ],
  [
    42,
    54
  ],
  [
    42,
    57
  ],
  [
    42,
    60
  ],
  [
    42,
    63
  ],
  [
    42,
    66
  ],
  [
    42,
    69
  ],
  [
    42,
    72
  ],
  [
    42,
    75
  ],
  [
    42,
    78
  ],
  [
    42,
    81
  ],
  [
    42,
    84
  ],
  [
    42,
    87
  ],
  [
    42,
    90
  ],
  [
    42,
    93
  ],
  [
    42,
    96
  ],
  [
    42,
    99
  ],
  [
    42,
    102
  ],
  [
    42,
    105
  ],
  [
    42,
    108
  ],
  [
    42,
    111
  ],
  [
    42,
    114
  ],
  [
    42,
    117
  ],
  [
    42,
    120
  ],
  [
    42,
    123
  ],
  [
    42,
    126
  ],
  [
    42,
    129
  ],
  [
    42,
    180
  ],
  [
    45,
    -180
  ],
  [
    45,
    -123
  ],
  [
    45,
    -120
  ],
  [
    45,
    -117
  ],
  [
    45,
    -114
  ],
  [
    45,
    -111
  ],
  [
    45,
    -108
  ],
  [
    45,
    -105
  ],
  [
    45,
    -102
  ],
  [
    45,
    -99
  ],
  [
    45,
    -96
  ],
  [
    45,
    -93
  ],
  [
    45,
    -90
  ],
  [
    45,
    -87
  ],
  [
    45,
    -84
  ],
  [
    45,
    -81
  ],
  [
    45,
    -78
  ],
  [
    45,
    -75
  ],
  [
    45,
    -72
  ],
  [
    45,
    -69
  ],
  [
    45,
    -63
  ],
  [
    45,
    0
  ],
  [
    45,
    3
  ],
  [
    45,
    6
  ],
  [
    45,
    9
  ],
  [
    45,
    12
  ],
  [
    45,
    15
  ],
  [
    45,
    18
  ],
  [
    45,
    21
  ],
  [
    45,
    24
  ],
  [
    45,
    27
  ],
  [
    45,
    39
  ],
  [
    45,
    42
  ],
  [
    45,
    45
  ],
  [
    45,
    54
  ],
  [
    45,
    57
  ],
  [
    45,
    60
  ],
  [
    45,
    63
  ],
  [
    45,
    66
  ],
  [
    45,
    69
  ],
  [
    45,
    72
  ],
  [
    45,
    75
  ],
  [
    45,
    78
  ],
  [
    45,
    81
  ],
  [
    45,
    84
  ],
  [
    45,
    87
  ],
  [
    45,
    90
  ],
  [
    45,
    93
  ],
  [
    45,
    96
  ],
  [
    45,
    99
  ],
  [
    45,
    102
  ],
  [
    45,
    105
  ],
  [
    45,
    108
  ],
  [
    45,
    111
  ],
  [
    45,
    114
  ],
  [
    45,
    117
  ],
  [
    45,
    120
  ],
  [
    45,
    123
  ],
  [
    45,
    126
  ],
  [
    45,
    129
  ],
  [
    45,
    132
  ],
  [
    45,
    135
  ],
  [
    45,
    180
  ],
  [
    48,
    -180
  ],
  [
    48,
    -123
  ],
  [
    48,
    -120
  ],
  [
    48,
    -117
  ],
  [
    48,
    -114
  ],
  [
    48,
    -111
  ],
  [
    48,
    -108
  ],
  [
    48,
    -105
  ],
  [
    48,
    -102
  ],
  [
    48,
    -99
  ],
  [
    48,
    -96
  ],
  [
    48,
    -93
  ],
  [
    48,
    -90
  ],
  [
    48,
    -87
  ],
  [
    48,
    -84
  ],
  [
    48,
    -81
  ],
  [
    48,
    -78
  ],
  [
    48,
    -75
  ],
  [
    48,
    -72
  ],
  [
    48,
    -69
  ],
  [
    48,
    -57
  ],
  [
    48,
    -54
  ],
  [
    48,
    -3
  ],
  [
    48,
    0
  ],
  [
    48,
    3
  ],
  [
    48,
    6
  ],
  [
    48,
    9
  ],
  [
    48,
    12
  ],
  [
    48,
    15
  ],
  [
    48,
    18
  ],
  [
    48,
    21
  ],
  [
    48,
    24
  ],
  [
    48,
    27
  ],
  [
    48,
    30
  ],
  [
    48,
    33
  ],
  [
    48,
    36
  ],
  [
    48,
    39
  ],
  [
    48,
    42
  ],
  [
    48,
    45
  ],
  [
    48,
    48
  ],
  [
    48,
    51
  ],
  [
    48,
    54
  ],
  [
    48,
    57
  ],
  [
    48,
    60
  ],
  [
    48,
    63
  ],
  [
    48,
    66
  ],
  [
    48,
    69
  ],
  [
    48,
    72
  ],
  [
    48,
    75
  ],
  [
    48,
    78
  ],
  [
    48,
    81
  ],
  [
    48,
    84
  ],
  [
    48,
    87
  ],
  [
    48,
    90
  ],
  [
    48,
    93
  ],
  [
    48,
    96
  ],
  [
    48,
    99
  ],
  [
    48,
    102
  ],
  [
    48,
    105
  ],
  [
    48,
    108
  ],
  [
    48,
    111
  ],
  [
    48,
    114
  ],
  [
    48,
    117
  ],
  [
    48,
    120
  ],
  [
    48,
    123
  ],
  [
    48,
    126
  ],
  [
    48,
    129
  ],
  [
    48,
    132
  ],
  [
    48,
    135
  ],
  [
    48,
    138
  ],
  [
    48,
    180
  ],
  [
    51,
    -180
  ],
  [
    51,
    -126
  ],
  [
    51,
    -123
  ],
  [
    51,
    -120
  ],
  [
    51,
    -117
  ],
  [
    51,
    -114
  ],
  [
    51,
    -111
  ],
  [
    51,
    -108
  ],
  [
    51,
    -105
  ],
  [
    51,
    -102
  ],
  [
    51,
    -99
  ],
  [
    51,
    -96
  ],
  [
    51,
    -93
  ],
  [
    51,
    -90
  ],
  [
    51,
    -87
  ],
  [
    51,
    -84
  ],
  [
    51,
    -81
  ],
  [
    51,
    -78
  ],
  [
    51,
    -75
  ],
  [
    51,
    -72
  ],
  [
    51,
    -69
  ],
  [
    51,
    -66
  ],
  [
    51,
    -63
  ],
  [
    51,
    -60
  ],
  [
    51,
    -3
  ],
  [
    51,
    0
  ],
  [
    51,
    3
  ],
  [
    51,
    6
  ],
  [
    51,
    9
  ],
  [
    51,
    12
  ],
  [
    51,
    15
  ],
  [
    51,
    18
  ],
  [
    51,
    21
  ],
  [
    51,
    24
  ],
  [
    51,
    27
  ],
  [
    51,
    30
  ],
  [
    51,
    33
  ],
  [
    51,
    36
  ],
  [
    51,
    39
  ],
  [
    51,
    42
  ],
  [
    51,
    45
  ],
  [
    51,
    48
  ],
  [
    51,
    51
  ],
  [
    51,
    54
  ],
  [
    51,
    57
  ],
  [
    51,
    60
  ],
  [
    51,
    63
  ],
  [
    51,
    66
  ],
  [
    51,
    69
  ],
  [
    51,
    72
  ],
  [
    51,
    75
  ],
  [
    51,
    78
  ],
  [
    51,
    81
  ],
  [
    51,
    84
  ],
  [
    51,
    87
  ],
  [
    51,
    90
  ],
  [
    51,
    93
  ],
  [
    51,
    96
  ],
  [
    51,
    99
  ],
  [
    51,
    102
  ],
  [
    51,
    105
  ],
  [
    51,
    108
  ],
  [
    51,
    111
  ],
  [
    51,
    114
  ],
  [
    51,
    117
  ],
  [
    51,
    120
  ],
  [
    51,
    123
  ],
  [
    51,
    126
  ],
  [
    51,
    129
  ],
  [
    51,
    132
  ],
  [
    51,
    135
  ],
  [
    51,
    138
  ],
  [
    51,
    180
  ],
  [
    54,
    -180
  ],
  [
    54,
    -132
  ],
  [
    54,
    -129
  ],
  [
    54,
    -126
  ],
  [
    54,
    -123
  ],
  [
    54,
    -120
  ],
  [
    54,
    -117
  ],
  [
    54,
    -114
  ],
  [
    54,
    -111
  ],
  [
    54,
    -108
  ],
  [
    54,
    -105
  ],
  [
    54,
    -102
  ],
  [
    54,
    -99
  ],
  [
    54,
    -96
  ],
  [
    54,
    -93
  ],
  [
    54,
    -90
  ],
  [
    54,
    -87
  ],
  [
    54,
    -84
  ],
  [
    54,
    -78
  ],
  [
    54,
    -75
  ],
  [
    54,
    -72
  ],
  [
    54,
    -69
  ],
  [
    54,
    -66
  ],
  [
    54,
    -63
  ],
  [
    54,
    -60
  ],
  [
    54,
    -9
  ],
  [
    54,
    9
  ],
  [
    54,
    12
  ],
  [
    54,
    15
  ],
  [
    54,
    18
  ],
  [
    54,
    21
  ],
  [
    54,
    24
  ],
  [
    54,
    27
  ],
  [
    54,
    30
  ],
  [
    54,
    33
  ],
  [
    54,
    36
  ],
  [
    54,
    39
  ],
  [
    54,
    42
  ],
  [
    54,
    45
  ],
  [
    54,
    48
  ],
  [
    54,
    51
  ],
  [
    54,
    54
  ],
  [
    54,
    57
  ],
  [
    54,
    60
  ],
  [
    54,
    63
  ],
  [
    54,
    66
  ],
  [
    54,
    69
  ],
  [
    54,
    72
  ],
  [
    54,
    75
  ],
  [
    54,
    78
  ],
  [
    54,
    81
  ],
  [
    54,
    84
  ],
  [
    54,
    87
  ],
  [
    54,
    90
  ],
  [
    54,
    93
  ],
  [
    54,
    96
  ],
  [
    54,
    99
  ],
  [
    54,
    102
  ],
  [
    54,
    105
  ],
  [
    54,
    108
  ],
  [
    54,
    111
  ],
  [
    54,
    114
  ],
  [
    54,
    117
  ],
  [
    54,
    120
  ],
  [
    54,
    123
  ],
  [
    54,
    126
  ],
  [
    54,
    129
  ],
  [
    54,
    132
  ],
  [
    54,
    135
  ],
  [
    54,
    156
  ],
  [
    54,
    159
  ],
  [
    54,
    180
  ],
  [
    57,
    -180
  ],
  [
    57,
    -135
  ],
  [
    57,
    -132
  ],
  [
    57,
    -129
  ],
  [
    57,
    -126
  ],
  [
    57,
    -123
  ],
  [
    57,
    -120
  ],
  [
    57,
    -117
  ],
  [
    57,
    -114
  ],
  [
    57,
    -111
  ],
  [
    57,
    -108
  ],
  [
    57,
    -105
  ],
  [
    57,
    -102
  ],
  [
    57,
    -99
  ],
  [
    57,
    -96
  ],
  [
    57,
    -93
  ],
  [
    57,
    -90
  ],
  [
    57,
    -75
  ],
  [
    57,
    -72
  ],
  [
    57,
    -69
  ],
  [
    57,
    -66
  ],
  [
    57,
    -63
  ],
  [
    57,
    -3
  ],
  [
    57,
    15
  ],
  [
    57,
    24
  ],
  [
    57,
    27
  ],
  [
    57,
    30
  ],
  [
    57,
    33
  ],
  [
    57,
    36
  ],
  [
    57,
    39
  ],
  [
    57,
    42
  ],
  [
    57,
    45
  ],
  [
    57,
    48
  ],
  [
    57,
    51
  ],
  [
    57,
    54
  ],
  [
    57,
    57
  ],
  [
    57,
    60
  ],
  [
    57,
    63
  ],
  [
    57,
    66
  ],
  [
    57,
    69
  ],
  [
    57,
    72
  ],
  [
    57,
    75
  ],
  [
    57,
    78
  ],
  [
    57,
    81
  ],
  [
    57,
    84
  ],
  [
    57,
    87
  ],
  [
    57,
    90
  ],
  [
    57,
    93
  ],
  [
    57,
    96
  ],
  [
    57,
    99
  ],
  [
    57,
    102
  ],
  [
    57,
    105
  ],
  [
    57,
    108
  ],
  [
    57,
    111
  ],
  [
    57,
    114
  ],
  [
    57,
    117
  ],
  [
    57,
    120
  ],
  [
    57,
    123
  ],
  [
    57,
    126
  ],
  [
    57,
    129
  ],
  [
    57,
    132
  ],
  [
    57,
    135
  ],
  [
    57,
    138
  ],
  [
    57,
    159
  ],
  [
    57,
    162
  ],
  [
    57,
    180
  ],
  [
    60,
    -180
  ],
  [
    60,
    -162
  ],
  [
    60,
    -159
  ],
  [
    60,
    -156
  ],
  [
    60,
    -153
  ],
  [
    60,
    -150
  ],
  [
    60,
    -141
  ],
  [
    60,
    -138
  ],
  [
    60,
    -135
  ],
  [
    60,
    -132
  ],
  [
    60,
    -129
  ],
  [
    60,
    -126
  ],
  [
    60,
    -123
  ],
  [
    60,
    -120
  ],
  [
    60,
    -117
  ],
  [
    60,
    -114
  ],
  [
    60,
    -111
  ],
  [
    60,
    -108
  ],
  [
    60,
    -105
  ],
  [
    60,
    -102
  ],
  [
    60,
    -99
  ],
  [
    60,
    -96
  ],
  [
    60,
    -75
  ],
  [
    60,
    -72
  ],
  [
    60,
    6
  ],
  [
    60,
    9
  ],
  [
    60,
    12
  ],
  [
    60,
    15
  ],
  [
    60,
    18
  ],
  [
    60,
    33
  ],
  [
    60,
    36
  ],
  [
    60,
    39
  ],
  [
    60,
    42
  ],
  [
    60,
    45
  ],
  [
    60,
    48
  ],
  [
    60,
    51
  ],
  [
    60,
    54
  ],
  [
    60,
    57
  ],
  [
    60,
    60
  ],
  [
    60,
    63
  ],
  [
    60,
    66
  ],
  [
    60,
    69
  ],
  [
    60,
    72
  ],
  [
    60,
    75
  ],
  [
    60,
    78
  ],
  [
    60,
    81
  ],
  [
    60,
    84
  ],
  [
    60,
    87
  ],
  [
    60,
    90
  ],
  [
    60,
    93
  ],
  [
    60,
    96
  ],
  [
    60,
    99
  ],
  [
    60,
    102
  ],
  [
    60,
    105
  ],
  [
    60,
    108
  ],
  [
    60,
    111
  ],
  [
    60,
    114
  ],
  [
    60,
    117
  ],
  [
    60,
    120
  ],
  [
    60,
    123
  ],
  [
    60,
    126
  ],
  [
    60,
    129
  ],
  [
    60,
    132
  ],
  [
    60,
    135
  ],
  [
    60,
    138
  ],
  [
    60,
    141
  ],
  [
    60,
    144
  ],
  [
    60,
    147
  ],
  [
    60,
    150
  ],
  [
    60,
    153
  ],
  [
    60,
    162
  ],
  [
    60,
    165
  ],
  [
    60,
    180
  ],
  [
    63,
    -180
  ],
  [
    63,
    -162
  ],
  [
    63,
    -159
  ],
  [
    63,
    -156
  ],
  [
    63,
    -153
  ],
  [
    63,
    -150
  ],
  [
    63,
    -147
  ],
  [
    63,
    -144
  ],
  [
    63,
    -141
  ],
  [
    63,
    -138
  ],
  [
    63,
    -135
  ],
  [
    63,
    -132
  ],
  [
    63,
    -129
  ],
  [
    63,
    -126
  ],
  [
    63,
    -123
  ],
  [
    63,
    -120
  ],
  [
    63,
    -117
  ],
  [
    63,
    -114
  ],
  [
    63,
    -111
  ],
  [
    63,
    -108
  ],
  [
    63,
    -105
  ],
  [
    63,
    -102
  ],
  [
    63,
    -99
  ],
  [
    63,
    -96
  ],
  [
    63,
    -93
  ],
  [
    63,
    -69
  ],
  [
    63,
    -66
  ],
  [
    63,
    -48
  ],
  [
    63,
    -45
  ],
  [
    63,
    -42
  ],
  [
    63,
    9
  ],
  [
    63,
    12
  ],
  [
    63,
    15
  ],
  [
    63,
    18
  ],
  [
    63,
    24
  ],
  [
    63,
    27
  ],
  [
    63,
    30
  ],
  [
    63,
    33
  ],
  [
    63,
    36
  ],
  [
    63,
    39
  ],
  [
    63,
    42
  ],
  [
    63,
    45
  ],
  [
    63,
    48
  ],
  [
    63,
    51
  ],
  [
    63,
    54
  ],
  [
    63,
    57
  ],
  [
    63,
    60
  ],
  [
    63,
    63
  ],
  [
    63,
    66
  ],
  [
    63,
    69
  ],
  [
    63,
    72
  ],
  [
    63,
    75
  ],
  [
    63,
    78
  ],
  [
    63,
    81
  ],
  [
    63,
    84
  ],
  [
    63,
    87
  ],
  [
    63,
    90
  ],
  [
    63,
    93
  ],
  [
    63,
    96
  ],
  [
    63,
    99
  ],
  [
    63,
    102
  ],
  [
    63,
    105
  ],
  [
    63,
    108
  ],
  [
    63,
    111
  ],
  [
    63,
    114
  ],
  [
    63,
    117
  ],
  [
    63,
    120
  ],
  [
    63,
    123
  ],
  [
    63,
    126
  ],
  [
    63,
    129
  ],
  [
    63,
    132
  ],
  [
    63,
    135
  ],
  [
    63,
    138
  ],
  [
    63,
    141
  ],
  [
    63,
    144
  ],
  [
    63,
    147
  ],
  [
    63,
    150
  ],
  [
    63,
    153
  ],
  [
    63,
    156
  ],
  [
    63,
    159
  ],
  [
    63,
    162
  ],
  [
    63,
    165
  ],
  [
    63,
    168
  ],
  [
    63,
    171
  ],
  [
    63,
    174
  ],
  [
    63,
    177
  ],
  [
    63,
    180
  ],
  [
    66,
    -180
  ],
  [
    66,
    -177
  ],
  [
    66,
    -174
  ],
  [
    66,
    -171
  ],
  [
    66,
    -165
  ],
  [
    66,
    -162
  ],
  [
    66,
    -159
  ],
  [
    66,
    -156
  ],
  [
    66,
    -153
  ],
  [
    66,
    -150
  ],
  [
    66,
    -147
  ],
  [
    66,
    -144
  ],
  [
    66,
    -141
  ],
  [
    66,
    -138
  ],
  [
    66,
    -135
  ],
  [
    66,
    -132
  ],
  [
    66,
    -129
  ],
  [
    66,
    -126
  ],
  [
    66,
    -123
  ],
  [
    66,
    -120
  ],
  [
    66,
    -117
  ],
  [
    66,
    -114
  ],
  [
    66,
    -111
  ],
  [
    66,
    -108
  ],
  [
    66,
    -105
  ],
  [
    66,
    -102
  ],
  [
    66,
    -99
  ],
  [
    66,
    -96
  ],
  [
    66,
    -93
  ],
  [
    66,
    -90
  ],
  [
    66,
    -87
  ],
  [
    66,
    -84
  ],
  [
    66,
    -72
  ],
  [
    66,
    -69
  ],
  [
    66,
    -63
  ],
  [
    66,
    -51
  ],
  [
    66,
    -48
  ],
  [
    66,
    -45
  ],
  [
    66,
    -42
  ],
  [
    66,
    -39
  ],
  [
    66,
    -36
  ],
  [
    66,
    -18
  ],
  [
    66,
    -15
  ],
  [
    66,
    15
  ],
  [
    66,
    18
  ],
  [
    66,
    21
  ],
  [
    66,
    24
  ],
  [
    66,
    27
  ],
  [
    66,
    30
  ],
  [
    66,
    33
  ],
  [
    66,
    42
  ],
  [
    66,
    45
  ],
  [
    66,
    48
  ],
  [
    66,
    51
  ],
  [
    66,
    54
  ],
  [
    66,
    57
  ],
  [
    66,
    60
  ],
  [
    66,
    63
  ],
  [
    66,
    66
  ],
  [
    66,
    69
  ],
  [
    66,
    72
  ],
  [
    66,
    75
  ],
  [
    66,
    78
  ],
  [
    66,
    81
  ],
  [
    66,
    84
  ],
  [
    66,
    87
  ],
  [
    66,
    90
  ],
  [
    66,
    93
  ],
  [
    66,
    96
  ],
  [
    66,
    99
  ],
  [
    66,
    102
  ],
  [
    66,
    105
  ],
  [
    66,
    108
  ],
  [
    66,
    111
  ],
  [
    66,
    114
  ],
  [
    66,
    117
  ],
  [
    66,
    120
  ],
  [
    66,
    123
  ],
  [
    66,
    126
  ],
  [
    66,
    129
  ],
  [
    66,
    132
  ],
  [
    66,
    135
  ],
  [
    66,
    138
  ],
  [
    66,
    141
  ],
  [
    66,
    144
  ],
  [
    66,
    147
  ],
  [
    66,
    150
  ],
  [
    66,
    153
  ],
  [
    66,
    156
  ],
  [
    66,
    159
  ],
  [
    66,
    162
  ],
  [
    66,
    165
  ],
  [
    66,
    168
  ],
  [
    66,
    171
  ],
  [
    66,
    174
  ],
  [
    66,
    177
  ],
  [
    66,
    180
  ],
  [
    69,
    -180
  ],
  [
    69,
    -162
  ],
  [
    69,
    -159
  ],
  [
    69,
    -156
  ],
  [
    69,
    -153
  ],
  [
    69,
    -150
  ],
  [
    69,
    -147
  ],
  [
    69,
    -144
  ],
  [
    69,
    -141
  ],
  [
    69,
    -138
  ],
  [
    69,
    -135
  ],
  [
    69,
    -132
  ],
  [
    69,
    -129
  ],
  [
    69,
    -126
  ],
  [
    69,
    -123
  ],
  [
    69,
    -120
  ],
  [
    69,
    -111
  ],
  [
    69,
    -108
  ],
  [
    69,
    -105
  ],
  [
    69,
    -102
  ],
  [
    69,
    -99
  ],
  [
    69,
    -96
  ],
  [
    69,
    -93
  ],
  [
    69,
    -84
  ],
  [
    69,
    -72
  ],
  [
    69,
    -69
  ],
  [
    69,
    -51
  ],
  [
    69,
    -48
  ],
  [
    69,
    -45
  ],
  [
    69,
    -42
  ],
  [
    69,
    -39
  ],
  [
    69,
    -36
  ],
  [
    69,
    -33
  ],
  [
    69,
    -30
  ],
  [
    69,
    -27
  ],
  [
    69,
    18
  ],
  [
    69,
    21
  ],
  [
    69,
    24
  ],
  [
    69,
    27
  ],
  [
    69,
    30
  ],
  [
    69,
    33
  ],
  [
    69,
    36
  ],
  [
    69,
    63
  ],
  [
    69,
    66
  ],
  [
    69,
    69
  ],
  [
    69,
    72
  ],
  [
    69,
    78
  ],
  [
    69,
    81
  ],
  [
    69,
    84
  ],
  [
    69,
    87
  ],
  [
    69,
    90
  ],
  [
    69,
    93
  ],
  [
    69,
    96
  ],
  [
    69,
    99
  ],
  [
    69,
    102
  ],
  [
    69,
    105
  ],
  [
    69,
    108
  ],
  [
    69,
    111
  ],
  [
    69,
    114
  ],
  [
    69,
    117
  ],
  [
    69,
    120
  ],
  [
    69,
    123
  ],
  [
    69,
    126
  ],
  [
    69,
    129
  ],
  [
    69,
    132
  ],
  [
    69,
    135
  ],
  [
    69,
    138
  ],
  [
    69,
    141
  ],
  [
    69,
    144
  ],
  [
    69,
    147
  ],
  [
    69,
    150
  ],
  [
    69,
    153
  ],
  [
    69,
    156
  ],
  [
    69,
    159
  ],
  [
    69,
    162
  ],
  [
    69,
    165
  ],
  [
    69,
    168
  ],
  [
    69,
    171
  ],
  [
    69,
    174
  ],
  [
    69,
    177
  ],
  [
    69,
    180
  ],
  [
    72,
    -180
  ],
  [
    72,
    -123
  ],
  [
    72,
    -117
  ],
  [
    72,
    -114
  ],
  [
    72,
    -111
  ],
  [
    72,
    -105
  ],
  [
    72,
    -99
  ],
  [
    72,
    -87
  ],
  [
    72,
    -84
  ],
  [
    72,
    -81
  ],
  [
    72,
    -78
  ],
  [
    72,
    -75
  ],
  [
    72,
    -54
  ],
  [
    72,
    -51
  ],
  [
    72,
    -48
  ],
  [
    72,
    -45
  ],
  [
    72,
    -42
  ],
  [
    72,
    -39
  ],
  [
    72,
    -36
  ],
  [
    72,
    -33
  ],
  [
    72,
    -30
  ],
  [
    72,
    -27
  ],
  [
    72,
    -24
  ],
  [
    72,
    54
  ],
  [
    72,
    69
  ],
  [
    72,
    72
  ],
  [
    72,
    75
  ],
  [
    72,
    84
  ],
  [
    72,
    87
  ],
  [
    72,
    90
  ],
  [
    72,
    93
  ],
  [
    72,
    96
  ],
  [
    72,
    99
  ],
  [
    72,
    102
  ],
  [
    72,
    105
  ],
  [
    72,
    108
  ],
  [
    72,
    111
  ],
  [
    72,
    114
  ],
  [
    72,
    117
  ],
  [
    72,
    120
  ],
  [
    72,
    123
  ],
  [
    72,
    126
  ],
  [
    72,
    129
  ],
  [
    72,
    141
  ],
  [
    72,
    144
  ],
  [
    72,
    147
  ],
  [
    72,
    180
  ],
  [
    75,
    -180
  ],
  [
    75,
    -111
  ],
  [
    75,
    -108
  ],
  [
    75,
    -96
  ],
  [
    75,
    -90
  ],
  [
    75,
    -87
  ],
  [
    75,
    -84
  ],
  [
    75,
    -81
  ],
  [
    75,
    -57
  ],
  [
    75,
    -54
  ],
  [
    75,
    -51
  ],
  [
    75,
    -48
  ],
  [
    75,
    -45
  ],
  [
    75,
    -42
  ],
  [
    75,
    -39
  ],
  [
    75,
    -36
  ],
  [
    75,
    -33
  ],
  [
    75,
    -30
  ],
  [
    75,
    -27
  ],
  [
    75,
    -24
  ],
  [
    75,
    -21
  ],
  [
    75,
    57
  ],
  [
    75,
    60
  ],
  [
    75,
    90
  ],
  [
    75,
    93
  ],
  [
    75,
    96
  ],
  [
    75,
    99
  ],
  [
    75,
    102
  ],
  [
    75,
    105
  ],
  [
    75,
    108
  ],
  [
    75,
    111
  ],
  [
    75,
    138
  ],
  [
    75,
    141
  ],
  [
    75,
    150
  ],
  [
    75,
    180
  ],
  [
    78,
    -180
  ],
  [
    78,
    -111
  ],
  [
    78,
    -96
  ],
  [
    78,
    -84
  ],
  [
    78,
    -81
  ],
  [
    78,
    -78
  ],
  [
    78,
    -72
  ],
  [
    78,
    -69
  ],
  [
    78,
    -66
  ],
  [
    78,
    -63
  ],
  [
    78,
    -60
  ],
  [
    78,
    -57
  ],
  [
    78,
    -54
  ],
  [
    78,
    -51
  ],
  [
    78,
    -48
  ],
  [
    78,
    -45
  ],
  [
    78,
    -42
  ],
  [
    78,
    -39
  ],
  [
    78,
    -36
  ],
  [
    78,
    -33
  ],
  [
    78,
    -30
  ],
  [
    78,
    -27
  ],
  [
    78,
    -24
  ],
  [
    78,
    15
  ],
  [
    78,
    18
  ],
  [
    78,
    180
  ],
  [
    81,
    -180
  ],
  [
    81,
    -93
  ],
  [
    81,
    -84
  ],
  [
    81,
    -81
  ],
  [
    81,
    -78
  ],
  [
    81,
    -75
  ],
  [
    81,
    -72
  ],
  [
    81,
    -69
  ],
  [
    81,
    -63
  ],
  [
    81,
    -60
  ],
  [
    81,
    -57
  ],
  [
    81,
    -54
  ],
  [
    81,
    -51
  ],
  [
    81,
    -48
  ],
  [
    81,
    -45
  ],
  [
    81,
    -42
  ],
  [
    81,
    -39
  ],
  [
    81,
    -36
  ],
  [
    81,
    -33
  ],
  [
    81,
    -30
  ],
  [
    81,
    -27
  ],
  [
    81,
    -24
  ],
  [
    81,
    -21
  ],
  [
    81,
    -18
  ],
  [
    81,
    -15
  ],
  [
    81,
    57
  ],
  [
    81,
    96
  ],
  [
    81,
    180
  ],
  [
    84,
    -180
  ],
  [
    84,
    180
  ],
  [
    87,
    -180
  ],
  [
    87,
    180
  ],
  [
    90,
    -180
  ],
  [
    90,
    -177
  ],
  [
    90,
    -174
  ],
  [
    90,
    -171
  ],
  [
    90,
    -168
  ],
  [
    90,
    -165
  ],
  [
    90,
    -162
  ],
  [
    90,
    -159
  ],
  [
    90,
    -156
  ],
  [
    90,
    -153
  ],
  [
    90,
    -150
  ],
  [
    90,
    -147
  ],
  [
    90,
    -144
  ],
  [
    90,
    -141
  ],
  [
    90,
    -138
  ],
  [
    90,
    -135
  ],
  [
    90,
    -132
  ],
  [
    90,
    -129
  ],
  [
    90,
    -126
  ],
  [
    90,
    -123
  ],
  [
    90,
    -120
  ],
  [
    90,
    -117
  ],
  [
    90,
    -114
  ],
  [
    90,
    -111
  ],
  [
    90,
    -108
  ],
  [
    90,
    -105
  ],
  [
    90,
    -102
  ],
  [
    90,
    -99
  ],
  [
    90,
    -96
  ],
  [
    90,
    -93
  ],
  [
    90,
    -90
  ],
  [
    90,
    -87
  ],
  [
    90,
    -84
  ],
  [
    90,
    -81
  ],
  [
    90,
    -78
  ],
  [
    90,
    -75
  ],
  [
    90,
    -72
  ],
  [
    90,
    -69
  ],
  [
    90,
    -66
  ],
  [
    90,
    -63
  ],
  [
    90,
    -60
  ],
  [
    90,
    -57
  ],
  [
    90,
    -54
  ],
  [
    90,
    -51
  ],
  [
    90,
    -48
  ],
  [
    90,
    -45
  ],
  [
    90,
    -42
  ],
  [
    90,
    -39
  ],
  [
    90,
    -36
  ],
  [
    90,
    -33
  ],
  [
    90,
    -30
  ],
  [
    90,
    -27
  ],
  [
    90,
    -24
  ],
  [
    90,
    -21
  ],
  [
    90,
    -18
  ],
  [
    90,
    -15
  ],
  [
    90,
    -12
  ],
  [
    90,
    -9
  ],
  [
    90,
    -6
  ],
  [
    90,
    -3
  ],
  [
    90,
    0
  ],
  [
    90,
    3
  ],
  [
    90,
    6
  ],
  [
    90,
    9
  ],
  [
    90,
    12
  ],
  [
    90,
    15
  ],
  [
    90,
    18
  ],
  [
    90,
    21
  ],
  [
    90,
    24
  ],
  [
    90,
    27
  ],
  [
    90,
    30
  ],
  [
    90,
    33
  ],
  [
    90,
    36
  ],
  [
    90,
    39
  ],
  [
    90,
    42
  ],
  [
    90,
    45
  ],
  [
    90,
    48
  ],
  [
    90,
    51
  ],
  [
    90,
    54
  ],
  [
    90,
    57
  ],
  [
    90,
    60
  ],
  [
    90,
    63
  ],
  [
    90,
    66
  ],
  [
    90,
    69
  ],
  [
    90,
    72
  ],
  [
    90,
    75
  ],
  [
    90,
    78
  ],
  [
    90,
    81
  ],
  [
    90,
    84
  ],
  [
    90,
    87
  ],
  [
    90,
    90
  ],
  [
    90,
    93
  ],
  [
    90,
    96
  ],
  [
    90,
    99
  ],
  [
    90,
    102
  ],
  [
    90,
    105
  ],
  [
    90,
    108
  ],
  [
    90,
    111
  ],
  [
    90,
    114
  ],
  [
    90,
    117
  ],
  [
    90,
    120
  ],
  [
    90,
    123
  ],
  [
    90,
    126
  ],
  [
    90,
    129
  ],
  [
    90,
    132
  ],
  [
    90,
    135
  ],
  [
    90,
    138
  ],
  [
    90,
    141
  ],
  [
    90,
    144
  ],
  [
    90,
    147
  ],
  [
    90,
    150
  ],
  [
    90,
    153
  ],
  [
    90,
    156
  ],
  [
    90,
    159
  ],
  [
    90,
    162
  ],
  [
    90,
    165
  ],
  [
    90,
    168
  ],
  [
    90,
    171
  ],
  [
    90,
    174
  ],
  [
    90,
    177
  ],
  [
    90,
    180
  ]
]
let landLookup = null;

/**
 * Returns whether the given point is in the sea or not.
 * @public
 * @param {number} lat  The latitude of the point.
 * @param {number} lng  The longitude of the point.
 * @return {boolean} True if the point is in the sea, false otherwise.
 */
function isSea(lat, lng) {
  // Round lat/ lng
  let lat_lu = Math.round(lat / 3) * 3
  let lng_lu = Math.round(lng / 3) * 3
  if (landLookup === null) {
    landLookup = {}
    for (let loc of table) {
      landLookup[loc[0].toString() + " " + loc[1].toString()] = true
    }
  }
  
  let key = lat_lu.toString() + " " + lng_lu.toString()
  if (key in landLookup) {
    // console.log(1)
    return false
  } else {
    // console.log(0)
    return true
  }
}

export default isSea;